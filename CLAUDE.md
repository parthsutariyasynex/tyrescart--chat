...
- **Auto-fetch fires only on an EMPTY/incomplete cache** — supplier, tc, products AND chat. Chat gets there by passing `CACHE_ANY_AGE` as `maxAgeMs` (any cached copy counts as fresh); the catalogue pages check their page counts directly. Anything cached is rendered as-is and never revalidated on its own — refreshing is the Sync buttons' job. tc and products briefly revalidated on a 5-minute TTL instead, which re-walked the whole catalogue every time the user returned to the page after a gap.
